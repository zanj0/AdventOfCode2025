...
void Part1(vector<string> &lines)
{
    ...
    for (auto &it : lines)
    {
        ...
        for (auto &it2 : parts)
        {
            vector<string> range = Split(it2, '-');

            if (range.size() != 2)
                continue;
            ranges.pb({ToLLI(range[0]), ToLLI(range[1])});
        }
    }
    ...
    for (auto &it : ranges)
    {
        max_high = max(max_high, it.ss);
    }

    lli max_digit = to_string(max_high).size();

    vector<lli> pow_10(MAX_DIGIT + 1);
    pow_10[0] = 1;
    ...
    for (lli len = 1; 2 * len <= max_digit && len <= MAX_DIGIT; len++)
    {
        lli multiplier = pow_10[len] + 1;
        lli low = pow_10[len - 1];
        lli high = pow_10[len] - 1;

        for (auto &it : ranges)
        {
            lli actual_low = max((it.ff + multiplier - 1) / multiplier, low);
            lli actual_high = min(it.ss / multiplier, high);

            if (actual_low > actual_high)
                continue;
            lli now = 0;
            lli cnt = actual_high - actual_low + 1;
            lli sum = actual_low + actual_high;
            if (cnt & 1)
            {
                now = cnt * (sum / 2);
            }
...
vector<lli> CalculateLPS(string s)
{
    vector<lli> lps(s.size());
    lli i = 1, j = 0;
    while (i < s.size())
    {
...
lli GetBlocks(lli x)
{
    string s;
    s = to_string(x);
    vector<lli> lps = CalculateLPS(s);
    lli prefix_length = lps.back();
    lli candidate_length = s.size() - prefix_length;
    if (prefix_length == 0)
        return 1;
    if (s.size() % candidate_length)
        return 1;
    lli ret = s.size() / candidate_length;
    return ret;
}

bool HasMore(lli x)
{
    return GetBlocks(x) >= 2;
}
...
void Part2(vector<string> &lines)
{
    ...
    for (auto &it : lines)
    {
        ...
        for (auto &it2 : parts)
        {
            ...
            for (lli i = ToLLI(range[0]); i <= ToLLI(range[1]); i++)
            {
                if (HasMore(i))
                {
                    ret += i;
