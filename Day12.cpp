...
void Part1(vector<string> &lines)
{
    ...
    for (auto &it : lines)
    {
        ...
        if (it.find("x") != string::npos)
        {
            ...
            vector<string> s2 = Split(s[0], 'x');
            vector<string> s3 = Split(s[1], ' ');
            ...
            v.pb(Data{ToLLI(s2[0]), ToLLI(s2[1]), cnt});
        }
    }
    lli ret = 0;
    for (auto &it : v)
    {
        lli can_have = (it.row / box_length) * (it.column / box_height);
        lli need = accumulate(it.cnt.begin(), it.cnt.end(), 0LL);
        ret += need <= can_have;
    }
    cout << ret << endl;
}
