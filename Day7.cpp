...
*/
int n, m;
vector<vector<int>> Bfs(pair<int, int> src, vector<string> &grid)
{
    queue<pair<int, int>> q;
    q.push(src);
    vector<vector<int>> dist(n, vector<int>(m, -1));
    dist[src.ff][src.ss] = 1;
    ...
    while (!q.empty())
    {
        ...
        int ni = node.ff + 1;
        int nj = node.ss;
        if (IsInside(ni, nj, n, m) && dist[ni][nj] == -1)
        {
            if (grid[ni][nj] == '.')
            {
                q.push({ni, nj});
...
void Part1(vector<string> &lines)
{
    ...
    vector<vector<int>> dist = Bfs(src, lines);
...
lli Dfs(lli i, lli j, vector<string> &grid, vector<vector<lli>> &dp)
{
    ...
    if (IsInside(ni, nj, n, m))
    {
        if (grid[ni][nj] == '.')
        {
            ret += Dfs(ni, nj, grid, dp);
        }
        else if (grid[ni][nj] == '^')
        {

            ret += Dfs(ni, nj - 1, grid, dp);
            ret += Dfs(ni, nj + 1, grid, dp);
        }
    }
    else
...
void Part2(vector<string> &lines)
{
    ...
    vector<vector<lli>> dp(200, vector<lli>(200, -1));
    cout << Dfs(src.ff, src.ss, lines, dp) << endl;
}
