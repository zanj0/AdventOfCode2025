...
void Part1(vector<string> &lines)
{
    ...
    vector<vector<lli>> distances;
    for (int i = 0; i < n; i++)
    {
        for (int j = i + 1; j < n; j++)
        {
            distances.pb({GetDistance(points[i], points[j]), i, j});
        }
    }
    ...
    for (int i = 0; i < n; i++)
    {
        lli par = uf.getParent(i);
        if (done.count(par))
            continue;
        done.insert(par);
        sizes.pb(uf.rank[par]);
    }
...
void Part2(vector<string> &lines)
{
    ...
    {
        if (uf.unite(distances[i][1], distances[i][2]))
            last = distances[i];
    }
