...
────────────────────────────────────────────────────────────────────────
*/
vector<pair<lli, lli>> MergeRanges(vector<pair<lli, lli>> &ranges)
{
    sort(ranges.begin(), ranges.end());
    vector<pair<lli, lli>> merged_ranges;

    for (auto &it : ranges)
    {
        if (!merged_ranges.empty() && merged_ranges.back().ss >= it.ff)
        {
            merged_ranges.back().ss = max(merged_ranges.back().ss, it.ss);
        }
        else
            merged_ranges.pb(it);
    }
...
void Part1(vector<string> &lines)
{
    ...
    vector<pair<lli, lli>> merged_ranges = MergeRanges(ranges);

    set<pair<lli, lli>> range_set = set<pair<lli, lli>>(merged_ranges.begin(), merged_ranges.end());

    lli ret = 0;

    for (auto &it : nums)
    {
        auto it2 = range_set.upper_bound({it, 0});
        if (it2 == range_set.begin())
            continue;
        it2--;
        ret += it2->ff <= it && it2->ss >= it;
    }
