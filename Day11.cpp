...
map<string, long long> dp1;

long long Dfs1(const string &node)
{
    if (node == "out")
        return 1;

    auto it = dp1.find(node);
    if (it != dp1.end())
        return it->second;
    ...
    if (git != graph.end())
    {
        for (const string &to : git->second)
        {
            ret += Dfs1(to);
        }
    }
...
map<tuple<string, bool, bool>, long long> dp2;

long long Dfs2(const string &node, bool seen_dac, bool seen_fft)
{
    auto key = make_tuple(node, seen_dac, seen_fft);
    auto it = dp2.find(key);
    if (it != dp2.end())
        return it->second;

    if (node == "out")
    {
        long long val = (seen_dac && seen_fft) ? 1LL : 0LL;
        dp2[key] = val;
        return val;
    }
    ...
    if (git != graph.end())
    {
        for (const string &to : git->second)
        {
            bool new_seen_dac = seen_dac || (to == "dac");
            bool new_seen_fft = seen_fft || (to == "fft");
            ret += Dfs2(to, new_seen_dac, new_seen_fft);
        }
    }
...
void BuildGraph(vector<string> &lines)
{
    graph.clear();
    for (auto &it : lines)
    {
        ...
        vector<string> nodes = Split(now[1], ' ');
        for (auto &to : nodes)
        {
            graph[now[0]].pb(to);
        }
...
void Part2(vector<string> &lines)
{
    ...
    long long ret = Dfs2("svr", false, false);
    cout << ret << endl;
}
